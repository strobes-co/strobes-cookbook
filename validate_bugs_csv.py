...
class BugsValidator:
    ...
    def is_valid_bug_data(cls, bug_data: Dict) -> bool:
        if not cls.has_valid_common_fields(bug_data):
            return False

        bug_level = int(bug_data.get('bug_level', 0))
        if bug_level == BugLevel.CODE:
            return cls.is_valid_code_bug(bug_data)
        elif bug_level == BugLevel.WEB:
            return cls.is_valid_web_bug(bug_data)
        elif bug_level == BugLevel.NETWORK:
            return cls.is_valid_network_bug(bug_data)
        elif bug_level == BugLevel.PACKAGE:
            return cls.is_valid_package_bug(bug_data)
        elif bug_level == BugLevel.CLOUD:
            return cls.is_valid_cloud_bug(bug_data)
        else:
            raise ValueError(f"Invalid bug level: {bug_level}")
    ...
    def is_valid_code_bug(bug_data: Dict) -> bool:
        required_fields = ['vulnerable_code', 'file_name', 'start_line_number', 'end_line_number']
        for field in required_fields:
            if not bug_data.get(field):
                raise ValueError(f"Missing required field for code bug: {field}")
        return True

    @staticmethod
    def is_valid_web_bug(bug_data: Dict) -> bool:
        if not bug_data.get('affected_endpoints'):
            raise ValueError("Missing required field for web bug: affected_endpoints")
        return True

    @staticmethod
    def is_valid_network_bug(bug_data: Dict) -> bool:
        if not bug_data.get('port'):
            raise ValueError("Missing required field for network bug: port")
    ...
    def is_valid_package_bug(bug_data: Dict) -> bool:
        if not bug_data.get('package_name'):
            raise ValueError("Missing required field for package bug: package_name")
        return True
    ...
    def is_valid_cloud_bug(cls, bug_data: Dict) -> bool:
        ...
        if not bug_data.get('region'):
            raise ValueError("Missing required field for cloud bug: region")

        if cloud_type == CloudType.AWS:
            if bug_data['region'] not in cls.cloud_options['aws_regions']:
                raise ValueError(f"Invalid AWS region: {bug_data['region']}")
            if not bug_data.get('aws_category'):
                raise ValueError("Missing required field for AWS: aws_category")
            if bug_data['aws_category'] not in cls.cloud_options['services']:
                raise ValueError(f"Invalid AWS category: {bug_data['aws_category']}")
            if not bug_data.get('aws_account_id'):
                raise ValueError("Missing required field for AWS: aws_account_id")

        elif cloud_type == CloudType.AZURE:
            if bug_data['region'] not in cls.cloud_options['azure_regions']:
                raise ValueError(f"Invalid Azure region: {bug_data['region']}")
            if not bug_data.get('azure_category'):
                raise ValueError("Missing required field for Azure: azure_category")
            if not bug_data.get('azure_resource'):
                raise ValueError("Missing required field for Azure: azure_resource")

        elif cloud_type == CloudType.GCP:
            if bug_data['region'] not in cls.cloud_options['gcp_regions']:
                raise ValueError(f"Invalid GCP region: {bug_data['region']}")
            if not bug_data.get('gcp_project_id'):
                raise ValueError("Missing required field for GCP: gcp_project_id")
            if not bug_data.get('gcp_resource_id'):
                raise ValueError("Missing required field for GCP: gcp_resource_id")
